...
class Channel(Message)
{
    ...
    {
        static assert(isWeaklyIsolated!Value,
            "Argument type " ~ Value.stringof ~ " is not safe to pass between threads.");
...
unittest
{
    ...
    {
        if (!input1.clear)
            results1 ~= input1.next;
        if (!input2.clear)
            results2 ~= input2.next;
    }
...
unittest
{
    ...
    while (!control.empty || !numbers.empty)
        yield;
